...
if 'eta' in filtered_df.columns:
    # Extract minutes for avg calculation (basic)
    try:
...
with tab1:
    ...
    with col_chart1:
        ...
        fig_price = px.histogram(filtered_df, x="price", color="platform", nbins=20, title="Price Distribution by Platform")
        st.plotly_chart(fig_price, use_container_width=True)
        
    with col_chart2:
        st.subheader("Platform Share")
        fig_pie = px.pie(filtered_df, names="platform", title="Product Count by Platform", hole=0.4)
...
with tab2:
    ...
    # Show Grid
    display_cols = ["image_url", "name", "price", "mrp", "discount", "eta", "platform", "product_url"]
